...
ti AS (
    SELECT terrai_nom, etnia_nome, fase_ti, geom_5880 AS g
...
qui AS (
    SELECT nm_comunid, nm_municip, nr_familia, geom_5880 AS g
    FROM quilombolas
    WHERE geom_5880 IS NOT NULL
),
...
SELECT
    ...
    ti_inf.ti_inf_nomes,
    uc_inf.uc_inf_nomes,
    -- geometria (4326 original, para mapa)
    pr.geom AS geom
FROM p
LEFT JOIN processos pr USING (processo)
