...
# # NOVO (Bloco G do 06 — protecao Art. 211/213, renovacao de PLG): opcional na
# # checagem abaixo (nao aborta o pipeline se ausente, mesmo padrao de
# # protocolos_licenca_ambiental), mas necessario para a faixa vermelha e a
# # reconstrucao de aptidao historica refletirem a protecao corretamente.
...
#   "titulo_vencido_renovacao_protocolada",  "FALSE",                  "Vencido, renovacao PLG protocolada",  "Titulo vencido, mas com pedido de renovacao da PLG (idevento 521) protocolado ate a data de vencimento e sem indeferimento (522) registrado — Art. 211/213, Portaria DNPM 155/2016, mantem o titulo em vigor ate manifestacao da ANM. Nao apto para operar automaticamente; requer revisao manual",
...
# AJUSTE (2026-07, flags binarias — pedido: filtro "Tipo de alerta" da aba 4
# refletir colunas binarias, nao so a flag encadeada): as mesmas 6 flags
# independentes calculadas por segmentos_aptidao_processo() (fonte unica,
# graficos_historico.R) sao carregadas junto com apto_na_data/motivo_nao_
# apto_na_data. Declaracao SEM segmento cobrindo a data leva o mesmo default
# cauteloso ja usado pro motivo categorico ("em_analise" / fase_de_tramitacao
# _ou_pesquisa): so flag_fase_nao_operacional = TRUE, as demais FALSE.
apto_na_data_df <- base_decl |>
  dplyr::select(cfem_id) |>
  dplyr::left_join(match_seg, by = "cfem_id") |>
  dplyr::mutate(
