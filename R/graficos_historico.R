...
#       next
#     }
#     # fase + status + licenca OK — falta so checar vigencia de titulo, que
#     # pode mudar DENTRO do proprio segmento de fase (por isso sub-particiona)
#     if (nrow(intervalos_titulo) == 0) {
#       # sem NENHUM titulo com publicacao+vencimento cadastrados — trata como
#       # "sem_data_a_revisar" (nao "titulo_vencido"), entao a protecao 211/213
#       # nao se aplica aqui (mesmo criterio de situacao_atual no 06).
#       segmentos[[length(segmentos) + 1]] <- tibble::tibble(
...
#         # anterior — ou seja, o vencimento nominal que abriu este buraco.
#         # Usa essa data como referencia pra checar protocolo de renovacao
#         # (art. 211/213). Ver buraco_protegido_211_213() acima.
...
#       g <- periodos_nao_apto_processo(p, serie_fase_status, situacao_documental, protocolos_licenca_ambiental, eventos_renovacao_plg)
...
#   gaps_p <- periodos_nao_apto_processo(processo_alvo, serie_fase_status, situacao_documental, protocolos_licenca_ambiental, eventos_renovacao_plg)

#   suspensao_p <- eventos_marcacao(ev_p, papeis = "SUSPENDE")
#   retomada_p  <- eventos_marcacao(ev_p, papeis = "RETOMA")
#   anulacao_p  <- eventos_marcacao(ev_p, papeis = "FECHA")

#   # --- shapes: faixas de gap (retangulos vermelhos) ---
#   shapes <- list()
#   if (!is.null(gaps_p) && nrow(gaps_p) > 0) {
...
segmentos_aptidao_processo <- function(processo_alvo, serie_fase_status, situacao_documental,
  ...
  for (i in seq_len(nrow(fs2))) {
    ...
    # dentro de [ini, fim], independente de fase/status/licenca ja terem
    # falhado — assim as 6 condicoes sao avaliadas cada uma por conta
    ...
    if (nrow(titulo_part) == 0) next  # defensivo — nao deveria ocorrer (cobertura+buracos sempre cobrem [ini,fim])
