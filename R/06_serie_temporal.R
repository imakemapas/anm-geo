...
#         # FIX (Art. 216 e equivalentes, achado 2026-07): "BAIXA" incluido —
#         # cobre "TORNA S/EFEITO BAIXA TRANSCRICAO..." (reversao de uma baixa
#         # de titulo/registro), mesmo raciocinio dos outros TORNA S/EFEITO.
...
#         # NOVO (Art. 216, Portaria 155/2016, e equivalentes em outros regimes
#         # — achado 2026-07): "BAIXA TRANSCRICAO" e o ato formal que encerra o
#         # registro do titulo (ausencia de renovacao, renovacao fora do prazo,
#         # mudanca de regime, ou area liberada para edital apos a baixa) — em
#         # TODOS os regimes (AUT PESQ, LICEN, CONC LAV, PLG, REG EXT), nao so
#         # PLG. Antes caia em NEUTRO_marco_informativo (nao fechava nada).
#         # "TORNA S/EFEITO BAIXA..." ja e capturado por RETOMA acima (ordem
#         # importa no case_when), exclusao aqui e so para clareza/robustez.
#         stringr::str_detect(S, "BAIXA TRANSCRI") & !stringr::str_detect(S, "TORNA S/EFEITO") ~ "FECHA",

#         # CORRECAO (Art. 211/213 Portaria 155/2016, 2026-07): "RENOVACAO DO
#         # PRAZO PLG INDEFERIDA PUBL" (idevento 522) caia na regra generica de
#         # MUDA_FASE abaixo, porque so continha "INDEFERIDA" e a regra de
#         # INDEFERIMENTO acima exige a palavra "INDEFERIMENTO" (nao cobre a
#         # variante "INDEFERIDA"). Efeito pratico: um indeferimento da
#         # renovacao da PLG era tratado como se fosse a propria renovacao
#         # concedida. Precisa vir ANTES da regra de renovacao PLG em
#         # MUDA_FASE (mesmo raciocinio do ALVARA NULO ja corrigido).
...
# # =============================================================================
# # Achado de investigacao (2026-07): art. 211 exige protocolo do pedido de
# # renovacao ate o ultimo dia de vigencia da PLG; se atendido, art. 213 §
# # unico mantem o titulo em vigor ate manifestacao definitiva da ANM (PUBL de
# # renovacao concedida = idevento 523, MUDA_FASE, ou indeferida = idevento
# # 522, agora FECHA, corrigido acima). Isso NAO fecha o gap sozinho: o
# # protocolo (521, papel PROTOC) fica de fora da maquina de estados por
# # design (PROTOC nunca efetiva sozinho) — sem esta extracao, o pipeline nao
# # tem NENHUM jeito de saber que existe um pedido de renovacao pendente
# # quando o titulo vence nominalmente.
# #
# # Extracao direta por idevento (nao por padrao de texto) — auditavel e
# # estavel, ja que os dois idevento foram confirmados manualmente no
# # dicionario oficial (dicionario_eventos_classificado.csv) para esta
# # investigacao especifica. Fonte unica: exportado em parquet para uso tanto
# # aqui (situacao_atual) quanto em graficos_historico.R (segmentos_aptidao_
# # processo), para nao duplicar a logica e nao divergir entre as duas — mesmo
# # principio ja aplicado ao dicionario de eventos classificado.
...
# # Por processo: data do protocolo de renovacao mais recente e se ja existe
# # QUALQUER indeferimento posterior a ele (retroativo — decisao do usuario:
# # indeferimento "nega e valida o vencimento", nao delimita so o intervalo
# # ate a data do indeferimento, invalida a protecao inteira daquele ciclo).
...
#     # PROTECAO ART. 211/213 (Bloco G): so vale se o titulo esta de fato
#     # vencido (senao a pergunta nem se coloca), o protocolo de renovacao
#     # (521) existe e foi feito ATE a data de vencimento do titulo atual
#     # (exigencia do art. 211 — protocolo apos o vencimento nao protege), e
#     # NAO ha indeferimento (522) registrado depois do protocolo (decisao do
#     # usuario: indeferimento nega e valida o vencimento, retroativo — nao ha
#     # protecao parcial).
...
#     # --- FLAGS BINARIAS INDEPENDENTES (aditivo — nao substitui apto_operar/
#     # motivo_nao_apto acima, que continuam com a cascata categorica de
#     # sempre). Cada flag e calculada sem parar na primeira que bater, para
#     # nao esconder sobreposicoes (ex.: processo sem licenca ambiental E com
#     # titulo vencido ao mesmo tempo — hoje so aparece "sem_licenca_ambiental_
#     # previa" no motivo categorico; aqui as duas ficam visiveis).
#     flag_fase_nao_operacional                  = !(fase_evento %in% FASES_QUE_OPERAM),
#     flag_status_nao_ativo                      = status_evento != "ATIVA",
#     flag_sem_licenca_ambiental_previa          = !licenca_antes_da_fase,
...
# # --- CHECK DE CONSISTENCIA: motivo categorico x flag binaria de maior
# # prioridade marcada TRUE (mesmo espirito da validacao de regressao dos 69
# # idevento acima — garante que as duas fontes calculadas em paralelo, uma
# # vez introduzidas, nao divirjam silenciosamente no futuro). So roda quando
# # motivo_nao_apto nao e NA (apto_operar == TRUE nao tem flag correspondente
# # por definicao).
...
# --- licença ambiental (Bloco C) ---
# dt_protocolo_licenca_ambiental (primeira data) NAO muda — continua sendo o
# que alimenta a regra de apto_operar (nunca_protocolou_lic_amb) mais abaixo.
# MUDANCA (2026-07): a regra deixou de comparar dt_protocolo_licenca_ambiental
# com dt_ultimo_evento_fase (timing "protocolou antes da fase?"). Agora e
# apenas existencia de protocolo em qualquer momento — "nunca protocolou
# licenca ambiental", simplificacao decidida pela usuaria.
# dt_ultimo_protocolo_licenca_ambiental / n_protocolos_licenca_ambiental sao
# NOVAS, so para consulta/relatorio — nao entram em nenhuma regra de
# elegibilidade. Nao ha evento de aprovacao/vencimento especifico de licenca
# ambiental no dicionario (confirmado em dicionario_eventos_classificado.csv
...
situacao_atual <- fase_atual |>
  ...
  dplyr::mutate(
    ...
    # SIMPLIFICADO (2026-07): antes comparava dt_protocolo_licenca_ambiental
    # com dt_ultimo_evento_fase (timing). Agora e so existencia de protocolo,
    # em qualquer momento — "nunca protocolou licenca ambiental".
