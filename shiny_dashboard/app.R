...
# #         motivo_buraco <- if (buraco_protegido_211_213(buracos$xmin[k] - 1)) {
# #           "titulo_vencido_renovacao_protocolada"
# #         } else {
# #           "titulo_vencido"
# #         }
# #         segmentos[[length(segmentos) + 1]] <- tibble::tibble(
# #           xmin = buracos$xmin[k], xmax = buracos$xmax[k], apto_na_data = "FALSE", motivo_nao_apto_na_data = motivo_buraco)
# #       }
# #     }
# #   }
...
# #                       min = cp_ano_min, max = cp_ano_max, value = c(max(cp_ano_min, 2025), cp_ano_max), step = 1, sep = "", ticks = FALSE)
# #           ),
# #           tags$hr(),
# #           uiOutput("cp_info"),
# #           tags$hr(),
...
# #                            "valor_total", "peso_total_kg", "valor_suspeito", "peso_suspeito_kg",
# #                            "n_declaracoes_periodo_nao_apto"), names(df))
# #       df[, cols, drop = FALSE]
# #     } else {
# #       termo <- trimws(input$cp_busca %||% "")
...
# #       linha("Valor arrecadado", if (!is.null(row$valor_suspeito) && !is.na(row$valor_suspeito))
# #         paste0(fmt_rs(row$valor_total), " total | ", fmt_rs(row$valor_suspeito), " suspeito") else fmt_rs(row$valor_total)),
# #       linha("Peso comercializado", if (!is.null(row$peso_suspeito_kg) && !is.na(row$peso_suspeito_kg))
# #         paste0(fmt_kg(row$peso_total_kg), " total | ", fmt_kg(row$peso_suspeito_kg), " suspeito") else fmt_kg(row$peso_total_kg)),
...
# #       downloadButton("cp_relatorio_docx", "Relatório (Word)", class = "btn btn-light btn-sm"),
# #       downloadButton("cp_dossie_xlsx", "Dossiê completo (Excel)", class = "btn btn-light btn-sm")
# #     )
# #   })
...
# #           val_txt <- if (!is.null(row$valor_suspeito) && !is.na(row$valor_suspeito))
# #             paste0(fmt_rs(row$valor_total), " total | ", fmt_rs(row$valor_suspeito), " suspeito") else fmt_rs(row$valor_total)
# #           kg_txt <- if (!is.null(row$peso_suspeito_kg) && !is.na(row$peso_suspeito_kg))
# #             paste0(fmt_kg(row$peso_total_kg), " total | ", fmt_kg(row$peso_suspeito_kg), " suspeito") else fmt_kg(row$peso_total_kg)
# #           doc <- par_relatorio(doc, paste0("Valor arrecadado: ", val_txt))
# #           doc <- par_relatorio(doc, paste0("Peso comercializado: ", kg_txt))
# #         }
# #       }
...
# #       doc <- officer::body_add_img(doc, tmp_valor, width = 9, height = 2.46)
# #       doc <- officer::body_add_img(doc, tmp_peso,  width = 9, height = 2.46)
...
# #           flextable::fontsize(size = TAMANHO_RELATORIO, part = "all") |>
# #           flextable::autofit()
...
# #       # Paisagem (pedido do usuario: graficos e tabela ficam melhores) —
# #       # fecha a secao unica do documento como landscape, o que aplica a
# #       # orientacao a TODO o conteudo anterior (truque padrao do officer,
# #       # ja que ainda nao existe nenhuma outra quebra de secao no documento).
...
#     # inicio do segmento de fase (>= ini), replicando a logica de timing que
#     # o 06_serie_temporal.R usava. Agora e so existencia de protocolo, em
#     # qualquer momento — consistente com flag_sem_licenca_ambiental_previa.
#     if (is.na(dt_primeiro_protocolo_lic)) {
#       segmentos[[length(segmentos) + 1]] <- tibble::tibble(
#         xmin = ini, xmax = fim, apto_na_data = "FALSE", motivo_nao_apto_na_data = "sem_licenca_ambiental_previa")
#       next
#     }
...
server <- function(input, output, session) {
  ...
  cp_lista_df <- reactive({
    ...
    if (identical(input$cp_modo, "inaptos")) {
      ...
      if (!is.null(input$cp_categoria) && length(input$cp_categoria) > 0 && !is.null(cfem_motivo_ref)) {
        motivos_sel <- cfem_motivo_ref$motivo[cfem_motivo_ref$rotulo %in% input$cp_categoria]
        # AJUSTE (2026-07, flags binarias): OR entre as colunas tem_flag_X
        # correspondentes aos motivos selecionados — nao depende mais da
        # flag encadeada (que so registra o primeiro motivo na ordem de
        # prioridade, mascarando outras condicoes simultaneas).
        cols_sel <- intersect(unname(MOTIVO_TO_FLAGCOL[motivos_sel]), names(df))
